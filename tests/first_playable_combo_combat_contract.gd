...
func _init() -> void:
	...
	assert(bool(planner_signature.get("proactive_pressure_planning", false)))
	assert(float(planner_signature.get("pressure_idle_seconds", 99.0)) <= 0.70)
	...
	var pursuit := MASTER_PURSUIT.new()
	var pursuit_signature: Dictionary = pursuit.presentation_signature()
	assert(bool(pursuit_signature.get("direct_player_pursuit", false)))
	assert(float(pursuit_signature.get("disconnect_distance", 9999.0)) <= 400.0)
	assert(bool(pursuit_signature.get("vertical_chase", false)))
	assert(float(pursuit_signature.get("vertical_threshold", 9999.0)) <= 120.0)
	assert(bool(pursuit_signature.get("stuck_recovery", false)))
	assert(float(pursuit_signature.get("stuck_seconds", 99.0)) <= 0.50)
	assert(bool(pursuit_signature.get("jump_recovery", false)))
	assert(bool(pursuit_signature.get("strategic_point_override_when_disconnected", false)))
	assert(bool(pursuit_signature.get("returns_control_in_combat_range", false)))
	assert(not bool(pursuit_signature.get("damage_changes", true)))
	assert(not bool(pursuit_signature.get("attribute_buffs", true)))
	pursuit.free()
	...
	assert(bool(hud_signature.get("legacy_climax_panel_hidden", false)))
	assert(bool(hud_signature.get("player_climax_in_compact_strip", false)))
	...
	assert(bool(identity_signature.get("master_pursuit_runtime", false)))
