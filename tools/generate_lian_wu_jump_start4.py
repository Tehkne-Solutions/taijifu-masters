...
def compose(source, bounds, masks, index):
    ...
    x_drive = 2 if index >= 2 else 0
    layers["torso"] = offset(layers["torso"], x_drive, q["body_y"] + q["takeoff"])
    layers["pelvis"] = offset(layers["pelvis"], x_drive, q["pelvis_y"] + q["takeoff"])
...
def main():
    ...
    if not source.is_file():
        print(f"VM02_A5_JUMP_START4=BLOCKED source_missing={source}")
        return 2
    try:
        canonical_identity = validate_source(source)
    except (OSError, ValueError) as exc:
        print(f"VM02_A5_JUMP_START4=BLOCKED canonical_visual_identity={exc}"); return 3
    actual = str(canonical_identity["file_sha256"])
    image = Image.open(source).convert("RGBA")
    if image.size != CANVAS:
        print(f"VM02_A5_JUMP_START4=BLOCKED canvas={image.size}")
        return 4
    bounds = alpha_bounds(image)
    masks = build_masks(image.size, bounds)
    source_baseline = bounds[3] - 1
    frames_dir = repo / "assets/pack_01_characters/lian_wu/frames/jump_start"
    meta_dir = repo / "assets/pack_01_characters/lian_wu/metadata"
    frames_dir.mkdir(parents=True, exist_ok=True)
    meta_dir.mkdir(parents=True, exist_ok=True)
    for stale in frames_dir.glob("char_lian_wu__jump_start__f*.png"):
        stale.unlink()
    records = []
    for i in range(FRAME_COUNT):
        frame = compose(image, bounds, masks, i)
        path = frames_dir / f"char_lian_wu__jump_start__f{i + 1:02d}.png"
        frame.save(path, format="PNG", compress_level=9)
        fb = alpha_bounds(frame)
        baseline = fb[3] - 1 if fb != (0, 0, 0, 0) else -1
        if i < 3 and baseline != source_baseline:
            print(f"VM02_A5_JUMP_START4=BLOCKED grounded_baseline frame={i + 1} baseline={baseline}")
            return 5
        if i == 3 and baseline >= source_baseline:
            print(f"VM02_A5_JUMP_START4=BLOCKED takeoff_not_airborne baseline={baseline}")
            return 6
    ...
    meta = {
        ...
        "source_character_lock": {"file": str(source.relative_to(repo)).replace("\\", "/"), "sha256": actual, "source_alpha_bounds": list(bounds), "baseline_y": source_baseline},
        "generation": {"method": "articulated_jump_launch_v1", "frame_count": 4, "frame_numbering": "one_based_f01_to_f04", "loop": False, "fps": FPS, "semantic_phases": ["contact", "compression", "impulse", "takeoff"], "transition_target": "jump_loop", "global_warp": False, "redraw": False},
        "frames": records,
        "gates": {"identity_source_hash": "pass", "canvas": "pass", "transparent_background": "pass", "ground_contact_f01_f03": "pass", "takeoff_f04": "pass", "godot_visual_review": "pending", "runtime_promotion": "blocked"},
    }
    mp = meta_dir / "jump_start.json"
    mp.write_text(json.dumps(meta, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print("VM02_A5_JUMP_START4=GENERATED")
    print(f"source_sha256={actual}")
    print(f"source_alpha_bounds={bounds}")
    print(f"baseline_y={source_baseline}")
    print("frames=4")
    for r in records:
        print(f"FRAME_PASS {r['index']:02d} {r['sha256']} {r['file']}")
    print(f"metadata={mp.relative_to(repo)}")
    print("VM02_A5_JUMP_START4_GODOT_REVIEW=PENDING")
    return 0
