...
if (-not $python) {
  Write-Host "VM02_A3_WALK8=BLOCKED_PYTHON_NOT_FOUND"
  exit 2
}

Write-Host "VM02_A3_PYTHON_RESOLVE=PASS"
...
if ($pillowProbe.ExitCode -ne 0) {
  Write-Host "VM02_A3_PILLOW=MISSING"
  ...
  if ($pipInstall.ExitCode -ne 0) {
    Write-Host "VM02_A3_WALK8=BLOCKED_PILLOW_INSTALL"
    exit 3
  }
  $pillowProbe = Invoke-NativeProbe -Command $python -Arguments @("-c", "import PIL; print(PIL.__version__)")
  if ($pillowProbe.ExitCode -ne 0) {
    Write-Host "VM02_A3_WALK8=BLOCKED_PILLOW_IMPORT_AFTER_INSTALL"
    exit 3
  }
}

$pillowVersion = ($pillowProbe.Output | Select-Object -Last 1)
Write-Host "VM02_A3_PILLOW=PASS"
if ($pillowVersion) { Write-Host "PILLOW_VERSION=$pillowVersion" }

$generator = Join-Path $RepoRoot "tools\generate_lian_wu_walk8.py"
if (-not (Test-Path $generator)) {
  Write-Host "VM02_A3_WALK8=BLOCKED_GENERATOR_MISSING"
  exit 4
}

& $python $generator --repo-root $RepoRoot
if ($LASTEXITCODE -ne 0) {
  Write-Host "VM02_A3_WALK8=BLOCKED_GENERATION"
  exit $LASTEXITCODE
}
...
if (Test-Path $validator) {
  ...
  Write-Host "VM02_A3_LOCOMOTION_VALIDATOR_EXIT=$validatorExit"
}

Write-Host "VM02_A3_WALK8=READY_FOR_GODOT_REVIEW"
