...
$c37Path = Join-Path $RepoRoot "tools\RUN-VM02-C37-TRAINING-RIVAL-MATERIALIZATION-GATE.ps1"

if (-not (Test-Path $planPath)) { throw "VM02_C38_REQUIRED_FILES=BLOCKED missing_plan" }
if (-not (Test-Path $c37Path)) { throw "VM02_C38_REQUIRED_FILES=BLOCKED missing_c37" }
...
Write-Host "VM02_C38_C37_HANDOFF=BEGIN"
& powershell -ExecutionPolicy Bypass -File $c37Path -RepoRoot $RepoRoot 2>&1 | Out-Host
$c37Exit = $LASTEXITCODE
if ($c37Exit -ne 0) { throw "VM02_C38_C37_HANDOFF=BLOCKED exit=$c37Exit" }
