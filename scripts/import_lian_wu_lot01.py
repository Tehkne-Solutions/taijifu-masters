...
Entradas esperadas:
- master.png
- idle.png (6 frames horizontais)
- walk.png (8 frames horizontais)
- run.png (8 frames horizontais)

O script recorta frames 128x128, valida RGBA/transparência, calcula hashes,
gera metadata JSON, atlas PNG/JSON e atualiza production-status.json.
"""
...
def validate_transparency(image: Image.Image, label: str) -> None:
    ...
    if minimum == 255:
        raise ValueError(f"{label}: imagem não possui pixels transparentes")
    if maximum == 0:
...
def crop_sheet(sheet_path: Path, animation: str, frame_count: int, output_dir: Path) -> list[Path]:
    ...
    expected_width = FRAME_SIZE[0] * frame_count
    if sheet.size != (expected_width, FRAME_SIZE[1]):
        raise ValueError(
            f"{animation}: dimensões {sheet.size}; esperado {(expected_width, FRAME_SIZE[1])}"
        )
...
def update_status(pack_root: Path) -> None:
    ...
    status_path = pack_root / "production-status.json"
...
def main() -> int:
    ...
    parser.add_argument("--input", type=Path, required=True, help="Pasta com master.png, idle.png, walk.png e run.png")
    parser.add_argument(
        "--pack-root",
        type=Path,
        default=Path("assets/pack_01_characters/lian_wu"),
    )
    ...
    master_target = source_dir / "char_lian_wu__master.png"
    master.save(master_target, "PNG", optimize=True)
