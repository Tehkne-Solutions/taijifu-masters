...
func _start_battle() -> void:
	...
	controls_label.text = "P1: A/D mover • W saltar • S varredura/queda • Q esquiva • F técnica • G empurrão • R defesa/aparo    |    P2: Setas • Num0 • Num1 • Num2 • Num3"
...
func _spawn_fighters() -> void:
	...
	player_one.parry_performed.connect(_on_parry_performed)
	...
	player_two.parry_performed.connect(_on_parry_performed)
...
func _on_parry_performed(_fighter: FighterController) -> void:
	center_label.text = "APARO PERFEITO • FU"
	await get_tree().create_timer(0.45).timeout
	if not _resetting_round:
		center_label.text = "RUÍNAS DO CAMINHO TRIPLO\nTAI • JI • FU"
