...
const ROOT := "res://assets/modular_fighters/base_01/_ci_eye_candidates"
const EYES := [
...
class CandidateProfile:
	extends RefCounted
	var profile_id: StringName = &"c60_pack_b_eye_candidate"
...
func _init() -> void:
	...
	for path in EYES:
		if not ResourceLoader.exists(path):
			failures.append("candidate_missing:%s" % path)
	...
	for eyes_path in EYES:
		...
		print("C60_GODOT_EYE_CANDIDATE=PASS eye=%s" % eyes_path.get_file())
		assembler.queue_free()

	if not failures.is_empty():
		for failure in failures:
			push_error("C60_PACK_B_EYES_RUNTIME=BLOCKED %s" % failure)
		quit(2)
		return

	print("C60_PACK_B_EYES_RUNTIME=PASS candidates=5")
