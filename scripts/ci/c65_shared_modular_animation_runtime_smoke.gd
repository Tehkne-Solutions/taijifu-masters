...
func _run() -> void:
	...
	# Selection remains fail-closed before battle runtime proves that the full
	# modular identity can be animated. C65 promotes only the fighter metadata
	# after successful assembly inside the real battle.
	...
	# No preset must still render through the canonical LOT01 and must not create
	# the modular overlay. This proves the new runtime remains an opt-in handoff.
