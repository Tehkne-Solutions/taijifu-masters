...
func _run() -> void:
	...
	selector.skin_selected.connect(_on_skin_selected)
	...
	if _last_signal != StringName(EXPECTED_IDS[-1]):
		failures.append("last_signal:%s" % String(_last_signal))
