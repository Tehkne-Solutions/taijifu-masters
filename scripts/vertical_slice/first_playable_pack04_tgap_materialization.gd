...
## This class only validates product-specific completeness after TGAP promotion.
## Tehkné Solutions
...
const CHECKSUMS_SCHEMA := "tehkne/taijifu-pack04-checksums/v1"
...
const CONTRACT_FILES := [MANIFEST_PATH, RUNTIME_MAP_PATH, APPROVAL_PATH, CHECKSUMS_PATH]
...
func _probe(loader: Node, generation: int) -> Dictionary:
	...
	var checksums_result := _read_json(String(contract_paths[CHECKSUMS_PATH]), CHECKSUMS_SCHEMA, "checksums")
	...
	var checksums := checksums_result.get("data", {}) as Dictionary
	if not _identity_matches(checksums):
		return _blocked(["checksums_identity_mismatch"], generation, pack)
	var checksum_files_variant: Variant = checksums.get("files", {})
	if not (checksum_files_variant is Dictionary):
		return _blocked(["checksums_files_invalid"], generation, pack)
	var checksum_files := checksum_files_variant as Dictionary
	if checksum_files.size() != EXPECTED_ASSET_COUNT:
		return _blocked(["checksums_asset_count:%d_of_%d" % [checksum_files.size(), EXPECTED_ASSET_COUNT]], generation, pack)
...
func _safe_png_path(value: String) -> bool:
	if value.is_empty() or value.is_absolute_path() or not value.to_lower().ends_with(".png"):
		return false
