...
const BASE_TEXTURE := preload("res://assets/modular_fighters/base_00/base_fighter_v1_master.png")
...
func _run() -> void:
	...
	_source_used_rect = image.get_used_rect()
	...
	if not left_ok or not right_ok:
		_fail("VM02_C54_MODULAR_ASSEMBLY=BLOCKED", 5)
		return
	...
	if abs(left_bottom - BENCH_BASELINE_Y) > 0.01 or abs(right_bottom - BENCH_BASELINE_Y) > 0.01:
		_fail("VM02_C54_BASELINE=BLOCKED left=%.3f right=%.3f" % [left_bottom, right_bottom], 9)
		return
	...
	if capture == null or capture.is_empty():
		_fail("VM02_C54_CAPTURE=BLOCKED viewport_unavailable", 10)
		return
	...
	if save_error != OK:
		_fail("VM02_C54_CAPTURE=BLOCKED save_error=%s" % save_error, 11)
		return
...
func _build_fighter(root_x: float, flipped: bool, caption: String) -> bool:
	...
	var assembler = AssemblerClass.new()
	...
	var failures: PackedStringArray = assembler.configure(profile)
	...
	if not assembler.attach_visual_module(&"body_base", sprite):
		return false

	for slot in [&"hair_front", &"torso_outer", &"weapon_main"]:
		var empty_module := Node2D.new()
		if not assembler.attach_visual_module(slot, empty_module):
			return false
