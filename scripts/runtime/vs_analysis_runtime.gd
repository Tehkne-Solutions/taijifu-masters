...
func play(player_one_loadout: Dictionary, player_two_loadout: Dictionary, config: Dictionary) -> void:
	...
	_title.text = "%s\n%s" % [
		String(report.get("headline", "ANÁLISE VS")),
...
func build_report(player_one_loadout: Dictionary, player_two_loadout: Dictionary, config: Dictionary) -> Dictionary:
	...
	return {
		...
		"p1_strengths": p1_analysis["strengths"],
		"p1_risks": p1_analysis["risks"],
		"p2_strengths": p2_analysis["strengths"],
		"p2_risks": p2_analysis["risks"]
	}
...
func _analyze_side(
	build: BuildProfile,
	opponent: BuildProfile,
	loadout: Dictionary,
	opponent_loadout: Dictionary,
	config: Dictionary
) -> Dictionary:
	...
	if tai_delta >= 7.0:
		strengths.append("Mobilidade e iniciativa Tai superiores")
	elif tai_delta <= -7.0:
		risks.append("Pode perder espaço e altura nas rotas Tai")
	if ji_delta >= 7.0:
		strengths.append("Maior estabilidade para contato, agarrão e postura")
	elif ji_delta <= -7.0:
		risks.append("Pressão Ji adversária ameaça postura e desarmamento")
	if fu_delta >= 7.0:
		strengths.append("Melhor leitura para transições, aparos e adaptação")
	elif fu_delta <= -7.0:
		risks.append("Menor margem para responder a mudanças de condição")
	...
	if route_index >= opponent_route_index + 5.0:
		strengths.append("A arena favorece seu caminho %s" % String(route_bias).to_upper())
	elif route_index + 5.0 <= opponent_route_index:
		risks.append("A arena favorece o caminho %s do adversário" % String(route_bias).to_upper())
	...
	if weapon_range > opponent_range:
		strengths.append("Maior alcance inicial com %s" % BattleLoadoutCatalog.weapon_label(weapon_id))
	elif weapon_range < opponent_range:
		risks.append("Precisa atravessar o alcance de %s" % BattleLoadoutCatalog.weapon_label(opponent_weapon_id))
	...
	if StringName(loadout.get("variant_id", &"")) != &"":
		strengths.append("Leva uma variação técnica treinada")
	if strengths.is_empty():
		strengths.append("Build equilibrada, sem dependência de uma única rota")
	if risks.is_empty():
		risks.append("Risco principal depende da execução e leitura do adversário")
...
func _append_element_context(element_id: StringName, opponent_id: StringName, strengths: Array[String], risks: Array[String]) -> void:
	if element_id == opponent_id:
		...
		return
	if element_id == &"water" and opponent_id == &"fire":
		strengths.append("Água pode extinguir queimaduras e gerar vapor")
	elif element_id == &"fire" and opponent_id == &"air":
		strengths.append("Ar adversário pode ampliar sua combustão")
	elif element_id == &"earth" and opponent_id == &"water":
		strengths.append("Terra pode transformar água em controle por lama")
	elif element_id == &"air" and opponent_id == &"earth":
		risks.append("Alvos ancorados resistem parte do deslocamento do ar")
	else:
		strengths.append("Elemento oferece condições situacionais de controle")
...
func _format_side(prefix: String, build: BuildProfile, loadout: Dictionary, analysis: Dictionary, color: Color) -> String:
	...
	lines.append("[center][color=#%s][font_size=25][b]%s — %s[/b][/font_size][/color][/center]" % [color_hex, prefix, build.character_name.to_upper()])
	...
	lines.append("[center]%s • %s[/center]\n" % [
		BattleLoadoutCatalog.weapon_label(StringName(loadout.get("primary_weapon_id", build.weapon_id))),
		String(loadout.get("element_id", build.element_id)).to_upper()
	])
...
func _build_interface() -> void:
	...
	_footer = Label.new()
	_footer.offset_left = 90.0
	_footer.offset_top = 612.0
	_footer.offset_right = 1190.0
	_footer.offset_bottom = 700.0
	...
	_footer.add_theme_color_override("font_color", Color(0.72, 0.80, 0.92))
...
func _create_report_panel(left: float, top: float, right: float, bottom: float, border_color: Color) -> RichTextLabel:
	...
	style.bg_color = Color(0.025, 0.034, 0.055, 0.98)
	...
	report.add_theme_font_size_override("normal_font_size", 15)
	report.add_theme_font_size_override("bold_font_size", 16)
