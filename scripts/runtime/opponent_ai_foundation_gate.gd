...
func _physics_process(delta: float) -> void:
	...
	hud_state.text = "AI: %s · %s | PLAYER: %s · %s" % [String(opponent.ai_state).to_upper(), String(opponent.attack_phase).to_upper(), String(player.current_technique_id), String(player.attack_phase).to_upper()]
	hud_metrics.text = "player_hp=%.1f ai_hp=%.1f ai_attacks=%d ai_hits=%d counter_hits=%d combo=%s" % [player_hp, float(opponent.health), int(opponent.attack_count), (1 if ai_hit_player else 0), int(opponent.hit_count), str(combo_complete)]

	if combo_complete and opponent.ai_state != "hitstun" and player.attack_phase == "idle" and elapsed > 1.5:
		_finish_gate()
	elif elapsed > 7.0:
		print("VM02_C8_WATCHDOG=BLOCKED ai=%s attack=%s player_hp=%.1f ai_hp=%.1f ai_hits=%d player_links=%d" % [String(opponent.ai_state), String(opponent.attack_phase), player_hp, float(opponent.health), int(opponent.hit_count), player_links.size()])
		get_tree().quit(6)
...
func _process_player_hit_on_ai() -> void:
	...
	for area in player_attack.get_overlapping_areas():
		if area == $Opponent/Hurtbox:
			...
			print("VM02_C8_PLAYER_HIT_AI=PASS link=%d technique=%s damage=%.2f" % [int(player.combo_index), String(player.current_technique_id), float(t.damage)])
			return
...
func _finish_gate() -> void:
	...
	if int(opponent.attack_count) < 1: failures.append("AI never attacked")
	...
	if int(opponent.hit_count) != 2: failures.append("AI did not receive exactly two counter hits")
	if int(opponent.reaction_count) < 2: failures.append("AI did not react to counter hits")
	if float(opponent.health) >= 100.0: failures.append("AI health unchanged")
	...
	print("VM02_C8_AI_ATTACK=%s attacks=%d" % [("PASS" if int(opponent.attack_count) >= 1 else "BLOCKED"), int(opponent.attack_count)])
	print("VM02_C8_AI_DAMAGE_PLAYER=%s hp=%.2f" % [("PASS" if ai_hit_player and player_hp < 100.0 else "BLOCKED"), player_hp])
	print("VM02_C8_AI_RECEIVES_COUNTER=%s hits=%d" % [("PASS" if int(opponent.hit_count) == 2 else "BLOCKED"), int(opponent.hit_count)])
	print("VM02_C8_AI_REACTION=%s reactions=%d" % [("PASS" if int(opponent.reaction_count) >= 2 else "BLOCKED"), int(opponent.reaction_count)])
