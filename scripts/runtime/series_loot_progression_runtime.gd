...
func choose_reward(player_index: int, choice_index: int) -> bool:
	...
	if inventory.size() >= MAX_INVENTORY:
		var protected_id := String(_protected_item_ids.get(player_index, ""))
		var remove_index := 0
		if String(inventory[0].get("id", "")) == protected_id and inventory.size() > 1:
			remove_index = 1
...
func _apply_defeat_loss(player_index: int) -> void:
	...
	for index in range(inventory.size()):
		if String(inventory[index].get("id", "")) != protected_id:
...
func discard_item(player_index: int, item_index: int) -> bool:
	...
	_inventories[player_index] = inventory
	if String(_protected_item_ids.get(player_index, "")) == String(removed.get("id", "")):
