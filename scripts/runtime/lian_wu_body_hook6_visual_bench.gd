...
func _ready() -> void:
	for index in range(1, 7):
		var path := "%s/char_lian_wu__ji_body_hook__f%02d.png" % [FRAME_DIR, index]
...
func _draw() -> void:
	...
	for i in range(_frames.size()):
		...
		if bounds.size == Vector2i.ZERO: continue
		var scale_factor := VISUAL_HEIGHT / float(bounds.size.y)
		var size := Vector2(bounds.size) * scale_factor
		var baseline_y := 520.0 if i < 3 else 890.0
		var center_x := POSITIONS[i].x + 100.0
		var dest := Rect2(Vector2(center_x - size.x * 0.5, baseline_y - size.y), size)
		draw_texture_rect_region(_frames[i], dest, Rect2(bounds), Color.WHITE)
		var label_color := Color(1.0,0.72,0.28) if i == 3 else Color(0.88,0.90,0.94)
