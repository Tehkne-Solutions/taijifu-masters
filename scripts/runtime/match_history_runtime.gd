...
@onready var comparison_runtime: SeriesComparisonRuntime = get_node("../SeriesComparisonRuntime")

var _active := false
var _result_mode := false
var _token := 0
var _filter_field_index := 0
var _filters: Dictionary = {"character_id": &"all", "arena_id": &"all", "result_id": &"all", "curation_id": &"all", "text_query": ""}
var _selected_index := 0
var _visible_records: Array[Dictionary] = []
var _comparison_ids: Array[String] = []
...
func _process(_delta: float) -> void:
	...
	if not preparation_runtime.is_active():
		close()
		return
	if Input.is_action_just_pressed(&"history_search"):
		_search_input.grab_focus()
		_search_input.select_all()
	if _search_input.has_focus():
		return
	...
	if Input.is_action_just_pressed(&"history_reset_filters"):
		_filters = {"character_id": &"all", "arena_id": &"all", "result_id": &"all", "curation_id": &"all", "text_query": ""}
		_search_input.text = ""
		_selected_index = 0
		_refresh_history()
	...
	if Input.is_action_just_pressed(&"history_compare_select"):
		_toggle_comparison_selection()
	if Input.is_action_just_pressed(&"history_compare_open"):
		_open_comparison()
...
func play_result(record: Dictionary) -> void:
	...
	_footer.text = "Histórico salvo • abra F3 para rever, pesquisar, comparar e classificar a série."
...
func set_filters_for_test(filters: Dictionary) -> void:
	_filters = {
		...
		"text_query": String(filters.get("text_query", ""))
	}
	_search_input.text = String(_filters["text_query"])
	_selected_index = 0
	_refresh_history()
...
func select_for_comparison_for_test(match_id: String) -> bool:
	if statistics_runtime.history.match_by_id(match_id).is_empty():
		return false
	if match_id in _comparison_ids:
		_comparison_ids.erase(match_id)
		return true
	if _comparison_ids.size() >= 2:
		_comparison_ids.pop_front()
	_comparison_ids.append(match_id)
	return true

func comparison_ids() -> Array[String]:
	return _comparison_ids.duplicate()
...
func build_history_report() -> String:
	...
	var query := String(_filters.get("text_query", ""))
	lines.append("[center]BUSCA: [color=#8ecff0]%s[/color][/center]" % (query if query != "" else "SEM TEXTO"))
	...
	lines.append("[center]Séries %d • Rounds %d • Média %.1f rounds • %.1fs por round • %d destaques • %d favoritas[/center]" % [
		int(aggregate.get("series", 0)), int(aggregate.get("rounds", 0)), float(aggregate.get("average_rounds", 0.0)),
		float(aggregate.get("average_round_seconds", 0.0)), int(aggregate.get("highlights", 0)), int(aggregate.get("favorites", 0))
	])
	lines.append("[center]P1 %d vitórias • P2 %d vitórias • KO %d • Tempo %d • Prorrogação %d[/center]\n" % [
		int(aggregate.get("p1_wins", 0)), int(aggregate.get("p2_wins", 0)), int(aggregate.get("ko_rounds", 0)),
		int(aggregate.get("time_rounds", 0)), int(aggregate.get("sudden_death_rounds", 0))
	])
	lines.append("[color=#f4d477][font_size=18][b]SÉRIES ENCONTRADAS[/b][/font_size][/color]")
	if _visible_records.is_empty():
		lines.append("[color=#8b99ad]Nenhuma série corresponde aos filtros e à busca.[/color]")
	else:
		for index in range(_visible_records.size()):
			var record := _visible_records[index]
			var match_id := String(record.get("match_id", ""))
			var selected_for_compare := "[color=#b98cff][C][/color]" if match_id in _comparison_ids else "   "
			var prefix := "[color=#ffd36b]▶[/color]" if index == _selected_index else " "
			lines.append("%s %s %s" % [prefix, selected_for_compare, _history_line(record)])
	return "\n".join(lines)
...
func build_result_report(record: Dictionary) -> String:
	...
	if players.size() >= winner_index and players[winner_index - 1] is Dictionary:
		var winner: Dictionary = players[winner_index - 1]
		winner_name = String(winner.get("profile_name", winner.get("character_name", winner_name))).to_upper()
	var lines: Array[String] = []
	...
	if totals.size() >= 2:
		...
		for stat in [["Dano causado", "damage_dealt"], ["Dano de postura", "posture_damage"], ["Acertos", "hits"], ["Aparos", "parries"], ["Quebras de postura", "posture_breaks"], ["Desarmes", "disarms"], ["Agarrões", "grabs"], ["Interações elementais", "elemental_interactions"]]:
	...
	for round_value in record.get("rounds", []):
		if round_value is Dictionary:
			...
			lines.append("• Round %d — P%d • %s • %.1fs • %d momentos" % [int(round_data.get("round_number", 1)), int(round_data.get("winner_index", 1)), String(round_data.get("reason", "KO")), float(round_data.get("duration_seconds", 0.0)), (round_data.get("highlights", []) as Array).size()])
	return "\n".join(lines)

func _refresh_history() -> void:
	if not is_instance_valid(_report):
		return
	_search_input.visible = not _result_mode
	_report.text = build_history_report()
	_footer.text = "F3 fecha • Ctrl+F busca • Tab filtro • H/Y valor • Page Up/Down série • Enter replay • C seleciona comparação • V compara • B favorita • Ctrl+1..4 tags • Backspace limpa"
...
func _filter_field_label(field_id: StringName) -> String:
	return {&"character_id": "PERSONAGEM", &"arena_id": "ARENA", &"result_id": "RESULTADO", &"curation_id": "CURADORIA"}.get(field_id, String(field_id).to_upper())

func _filter_value_label(field_id: StringName, value_id: StringName) -> String:
	if value_id == &"all": return "TODOS"
	match field_id:
		...
		&"result_id": return {&"p1_win": "VITÓRIA P1", &"p2_win": "VITÓRIA P2", &"ko": "COM KO", &"time": "COM TEMPO", &"sudden_death": "COM PRORROGAÇÃO"}.get(value_id, String(value_id).to_upper())
...
func _toggle_comparison_selection() -> void:
	var selected := selected_record()
	if selected.is_empty(): return
	select_for_comparison_for_test(String(selected.get("match_id", "")))
	_refresh_history()

func _open_comparison() -> void:
	if _comparison_ids.size() != 2:
		return
	var left := statistics_runtime.history.match_by_id(_comparison_ids[0])
	var right := statistics_runtime.history.match_by_id(_comparison_ids[1])
	if comparison_runtime.open_comparison(left, right):
		close()
...
func _history_line(record: Dictionary) -> String:
	...
	if players.size() >= 2:
		if players[0] is Dictionary:
			var left: Dictionary = players[0]
			p1 = "%s/%s" % [String(left.get("profile_name", "P1")), String(left.get("character_name", ""))]
		if players[1] is Dictionary:
			var right: Dictionary = players[1]
			p2 = "%s/%s" % [String(right.get("profile_name", "P2")), String(right.get("character_name", ""))]
	var config: Dictionary = record.get("config", {})
	...
	for value in record.get("tags", []): tags.append(_tag_label(StringName(value)))
	var tag_suffix := " • [%s]" % ", ".join(tags) if not tags.is_empty() else ""
	return "%s%s %d — %d %s • vencedor P%d • %s • %d rounds • %d destaques%s" % [favorite, p1.to_upper(), int(record.get("score_p1", 0)), int(record.get("score_p2", 0)), p2.to_upper(), int(record.get("winner_index", 0)), CompetitiveMatchCatalog.arena_label(config), (record.get("rounds", []) as Array).size(), highlight_count, tag_suffix]

func _tag_label(tag_id: StringName) -> String:
	return {&"technical": "TÉCNICA", &"rematch": "REVANCHE", &"tournament": "TORNEIO", &"highlight": "DESTAQUE"}.get(tag_id, String(tag_id).to_upper())
...
func _build_interface() -> void:
	...
	_search_input.placeholder_text = "Ctrl+F • buscar perfil, personagem, build, arena, tag ou destaque"
	_search_input.max_length = 80
	_search_input.text_changed.connect(func(value: String) -> void:
		_filters["text_query"] = value
		_selected_index = 0
		_refresh_history()
	)
	_search_input.text_submitted.connect(func(_value: String) -> void: _search_input.release_focus())
	_search_input.gui_input.connect(func(event: InputEvent) -> void:
		if event is InputEventKey and event.pressed and event.keycode == KEY_ESCAPE:
			_search_input.release_focus()
	)
	column.add_child(_search_input)
	_report = RichTextLabel.new()
	_report.custom_minimum_size = Vector2(1080.0, 500.0)
...
func _register_inputs() -> void:
	...
	_add_key_action(&"history_search", KEY_F, true)
	...
	_add_key_action(&"history_compare_select", KEY_C)
	_add_key_action(&"history_compare_open", KEY_V)
...
func _add_key_action(action_id: StringName, keycode: Key, ctrl_pressed: bool = false) -> void:
	if not InputMap.has_action(action_id): InputMap.add_action(action_id, 0.5)
	for existing in InputMap.action_get_events(action_id):
		if existing is InputEventKey and existing.physical_keycode == keycode and existing.ctrl_pressed == ctrl_pressed: return
