...
## VM02-A3 visual review bench for Lian Wu Walk 8.
## Tehkné Solutions

const LOGICAL_SIZE := Vector2i(1280, 720)
const OUTPUT_SIZE := Vector2i(1920, 1080)
const OUTPUT_PATH := "res://artifacts/vm02-a3/lian-wu-walk8-bench-1920x1080.png"
const FRAME_DIR := "res://assets/pack_01_characters/lian_wu/frames/walk"
const FRAME_COUNT := 8
...
func _build_header() -> void:
	var title := Label.new()
	title.text = "VM02-A3 — LIAN WU WALK 8 / GODOT VISUAL BENCH"
	title.position = Vector2(48, 24)
	title.add_theme_font_size_override("font_size", 22)
	add_child(title)

	var subtitle := Label.new()
	subtitle.text = "Tehkné Solutions · articulated regional composite · gait/pivot/baseline review"
...
func _frame_path(frame_number: int) -> String:
	return "%s/char_lian_wu__walk__f%02d.png" % [FRAME_DIR, frame_number]
...
func _capture_after_frames() -> void:
	...
	print("VM02_A3_WALK8_BENCH_RUNTIME=%s" % ("PASS" if report.failures.is_empty() else "BLOCKED"))
	...
	var absolute_dir := ProjectSettings.globalize_path("res://artifacts/vm02-a3")
	...
	if source_size != OUTPUT_SIZE:
		...
		print("VM02_A3_WALK8_BENCH_NORMALIZED=PASS")
	var error := image.save_png(ProjectSettings.globalize_path(OUTPUT_PATH))
	if error != OK:
		push_error("failed to save walk8 visual bench PNG: %s" % error)
		get_tree().quit(4)
		return
	print("VM02_A3_WALK8_BENCH_CAPTURE=PASS")
	print("VM02_A3_WALK8_BENCH_OUTPUT=%s" % OUTPUT_PATH)
	get_tree().quit(0)
